...
## Сервер запустился, а клиент не подключается

Если в логах есть `Done (...)! For help, type "help"` и `Opening server on 0.0.0.0:19132`,
ядро работает, и проблема в сети. Порядок действий:

```bash
make diag                 # полная диагностика с вердиктом
make diag PORT=19133      # если сервер на другом порту
```

`scripts/diag.sh` проверяет по шагам: контейнер и его режим сети, занят ли UDP-порт
на хосте, отвечает ли сервер на RakNet-пинг локально, какой адрес выдал бот,
какой у машины внешний IP (и не CGNAT ли это), состояние ufw и агента playit.

`scripts/raknet_ping.py` — тот же запрос, который делает клиент 1.1.5. Запускается где угодно,
где есть python3 (в том числе на ноутбуке), и различает "сервер молчит" и "пакеты не доходят":

```bash
python3 scripts/raknet_ping.py                  # локально на сервере
python3 scripts/raknet_ping.py 203.0.113.10 19132   # снаружи
```

| Что показала диагностика | Что делать |
| --- | --- |
| локальный пинг не проходит | смотри `docker logs`, проверь `SERVER_NETWORK_MODE=host` |
| локально отвечает, снаружи нет, IP публичный | открой UDP 19132-19160 в панели провайдера + `sudo make firewall` |
| IP вида 100.64.x.x / 10.x.x.x | CGNAT или NAT: включай туннель `PLAYIT_SECRET` + `ADDRESS_MODE=playit` |
| всё зелёное, а игра молчит | в клиенте адрес и порт вводятся в РАЗНЫЕ поля (Add Server), без `:` |

Два сообщения в логе — косметика и на подключение не влияют:
`ifconfig: not found` (в образ добавлен `net-tools`) и `Debugging assertions are enabled`
(в запуск добавлен `-dzend.assertions=-1`).

## Временный адрес без ключа и регистрации

Если `scripts/diag.sh` пишет `За NAT` (публичного IP нет ни на одном интерфейсе),
нужен туннель. Вариант без аккаунта, без ключа и без подтверждения почты - **Pinggy** (UDP):

```bash
bash scripts/tunnel-nokey.sh            # поднять туннель и отдать адрес боту
bash scripts/tunnel-nokey.sh --watch    # держать адрес свежим (free = 60 минут)
make tunnel-nokey
```

Что делает скрипт:

1. запускает контейнер `mcpe-tunnel` (`node:22-alpine` + `npm i -g pinggy`) в сети хоста;
2. создаёт UDP-туннель: `pinggy --type udp -l 19132` (токен не передаётся);
3. вытаскивает выданный `host:port` из логов (`scripts/parse_tunnel_address.py`);
4. пишет в `.env`: `ADDRESS_MODE=manual`, `PUBLIC_HOST=<host:port>` и перезапускает бота;
5. проверяет адрес через `scripts/raknet_ping.py`.

Ограничения free-тарифа Pinggy: сессия 60 минут, порт меняется при переподключении,
один туннель на IP. Режим `--watch` перезапускает туннель и сам обновляет адрес у бота,
так что кнопка «Адрес» в Телеграме всегда показывает актуальные адрес и порт.

Альтернатива без любых сервисов - **Holesail** (P2P, TCP/UDP, без аккаунта и без лимита времени):

```bash
# на сервере
npx holesail --live 19132 --host 127.0.0.1 --udp     # покажет ключ hs://...
# у каждого игрока на ПК
npx holesail hs://<ключ> --port 19132 --udp        # дальше заход на 127.0.0.1:19132
```

Минус Holesail: клиент нужен на устройстве каждого игрока (на телефоне с MCPE это неудобно),
поэтому по умолчанию используется Pinggy. Сверь флаги командой `npx holesail --help`.
